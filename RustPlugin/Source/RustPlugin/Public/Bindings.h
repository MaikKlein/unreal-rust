...
using SpawnActorWithClassFn = uint32_t(*)(const UClassOpague* actor_class, UnrealTransform transform,
                                          AActorOpaque** out);
...
using SweepFn = uint32_t(*)(Vector3 start, Vector3 end, Quaternion rotation, LineTraceParams params,
                            CollisionShape collision_shape, HitResult* result);

using SweepMultiFn = uint32_t(*)(Vector3 start, Vector3 end, Quaternion rotation, LineTraceParams params,
                                 CollisionShape collision_shape, uintptr_t max_results, HitResult* results);

using OverlapMultiFn = uint32_t(*)(CollisionShape collision_shape, Vector3 position, Quaternion rotation,
                                   LineTraceParams params, uintptr_t max_results, OverlapResult* result);
...
using VisualLogCapsuleFn = void(*)(Utf8Str category, const AActorOpaque* owner, Vector3 position, Quaternion rotation,
                                   float half_height, float radius, Color color);

using VisualLogLocationFn = void(*)(Utf8Str category, const AActorOpaque* owner, Vector3 position, float radius,
                                    Color color);
...
using GetEditorComponentUObjectFn = uint32_t(*)(const AActorOpaque* actor, Uuid uuid, Utf8Str field, UObjectType ty,
                                                UObjectOpague** out);
...
using PlaySoundAtLocationFn = void(*)(const USoundBaseOpague* sound, Vector3 location, Quaternion rotation,
