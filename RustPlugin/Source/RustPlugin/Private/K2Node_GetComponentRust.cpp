...
void UK2Node_GetComponentRust::AllocateDefaultPins()
{
	...
	const auto ElementPin = CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Class, UObject::StaticClass(),
	                                  TEXT("EntityId"));
	const auto Out = CreatePin(EGPD_Output, UEdGraphSchema_K2::PC_Struct, TBaseStructure<FVector>::Get(),
	                           TEXT("Velocity"));
...
void UK2Node_GetComponentRust::OnUuidPicked(FString* Name)
{
	UE_LOG(LogTemp, Warning, TEXT("Picked %s"), **Name);
}
