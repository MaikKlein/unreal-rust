...
void FDynamicRustComponent2::Initialize(TSharedPtr<IPropertyHandle> Handle, FGuid InitGuid)
{
	TSharedPtr<IPropertyHandle> NameProperty = Handle->GetChildHandle(
		GET_MEMBER_NAME_CHECKED(FDynamicRustComponent2, Name));
	TSharedPtr<IPropertyHandle> RustPropertyMap = Handle->GetChildHandle(
		GET_MEMBER_NAME_CHECKED(FDynamicRustComponent2, Fields));
	...
	for (uint32_t Idx = 0; Idx < NumberOfFields; ++Idx)
	{
		...
		auto HandleTag = RustPropertyEntry->GetChildHandle(GET_MEMBER_NAME_CHECKED(FRustProperty2, Tag));
		...
		if (Type == ReflectionType::Float)
		{
			...
			auto HandleFloat = RustPropertyEntry->GetChildHandle(GET_MEMBER_NAME_CHECKED(FRustProperty2, Float));
		}
...
void FDynamicRustComponent2::Render(TSharedRef<IPropertyHandle> MapHandle, IDetailCategoryBuilder& DetailBuilder,
	...
	for (uint32 ComponentIdx = 0; ComponentIdx < NumberOfComponents; ++ComponentIdx)
	{
		...
		for (uint32 FieldIdx = 0; FieldIdx < NumberOfFields; ++FieldIdx)
		{
			...
			TSharedPtr<IPropertyHandle> TagProperty = RustPropertyEntry->GetChildHandle(
				GET_MEMBER_NAME_CHECKED(FRustProperty2, Tag));
			...
			if (Tag == ERustPropertyTag::Float)
			{
				auto FloatProperty = RustPropertyEntry->GetChildHandle(
					GET_MEMBER_NAME_CHECKED(FRustProperty2, Float));
				W.ValueContent()[
			...
			if (Tag == ERustPropertyTag::Vector)
			{
				auto VectorProperty = RustPropertyEntry->GetChildHandle(
					GET_MEMBER_NAME_CHECKED(FRustProperty2, Vector));
			...
			if (Tag == ERustPropertyTag::Bool)
			{
				auto BoolProperty = RustPropertyEntry->GetChildHandle(
					GET_MEMBER_NAME_CHECKED(FRustProperty2, Bool));
				W.ValueContent()[
			...
			{
				auto BoolProperty = RustPropertyEntry->GetChildHandle(
					GET_MEMBER_NAME_CHECKED(FRustProperty2, Rotation));
				W.ValueContent()[
					BoolProperty->CreatePropertyValueWidget()
				];
			}
			...
			{
				auto BoolProperty = RustPropertyEntry->GetChildHandle(
					GET_MEMBER_NAME_CHECKED(FRustProperty2, Class));
				W.ValueContent()[
					BoolProperty->CreatePropertyValueWidget()
				];
			}
